...
import { Inbox, Search, CheckCircle, FlaskConical, Loader2 } from "lucide-react"
import {
  Card,
  CardContent,
  CardDescription,
...
export function BsolventeDashboard() {
  const { sats, isLoading } = useSats({ laboratorio: SATDestino.BASE_SOLVENTE })

  const recebidas = useMemo(
    () => sats.filter((s) => s.status === SATStatus.ENVIADO_BSOLVENTE),
    [sats]
  )
  ...
  if (isLoading) {
    return (
      <PageTemplate title="Dashboard — Base Solvente" description="Carregando...">
    ...
    >
      ...
      <Card>
        <CardHeader className="flex flex-row items-center gap-2">
          ...
          <div>
            <CardTitle className="text-base">SATs Base Solvente</CardTitle>
            <CardDescription>
              Todas as SATs direcionadas para o laboratório de Base Solvente
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent>
